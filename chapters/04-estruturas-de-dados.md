...
Correlação é uma medida de dependência entre variáveis ("variáveis", no sentido estatístico, e não o sentido JavaScript). É geralmente expressa em um coeficiente que varia de -1 a 1. Correlação de _zero_ significa que as variáveis não estão relacionadas, enquanto uma correlação de _um_ indica que ambas estão perfeitamente relacionadas - se você conhece uma, você também conhece a outra. _Menos um_ também significa que as variáveis estão perfeitamente ligadas, mas que são opostas uma à outra, quando uma deles é verdadeira, a outra é falsa.
