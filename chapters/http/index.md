...
<pre data-language="message/http" class="snippet cm-s-default">
...
Essa simples abstração torna mais fácil usar `XMLHttpRequest` para simples requisições GET. Se você está escrevendo um programa que precisa fazer requisições HTTP, é uma boa idéia usar uma função helper para que você não acabe repetindo o esquisito padrão `XMLHttpRequest` por todo o código.

O nome da função argumento, `callback`, é um termo normalmente usado para descrever funções como essa. Uma função `callback` é fornecida a outro código para prover a este código uma forma de "ser chamado" mais tarde.
...
normalmente temos que procrastinar parte de nosso trabalho o colocando em uma função `callback`, o escopo de um bloco try se torna sem sentido. No seguinte código, a exceção não será pega porque a chamada para backgroundReadFile retorna imediatamente. O controle então deixa o bloco `try`, e a função que foi determinada não será chamada até mais tarde.
...
Para manusear requisições que falharam, temos que permitir uma função adicional para ser passada para nosso `wrapper`
e o chamar quando a requisição falhar. Alternativamente, podemos usar a convenção de que caso a requisição falhe, um argumento adicional descrevendo o problema é passado à chamada regular da função `callback`. Segue um exemplo:
...
Nós adicionamos uma função manipuladora para o evento "error", o qual será sinalizado quando a requisição falhar por completo. Nós também chamamos a função de `callback ` com um argumento de erro quando a requisição completa com um código de status que indica um erro.
...
Para projetos complexos, escrever código assíncrono no tradicional estilo de `callbacks` é difícil de ser feito corretamente. É fácil de esquecer a checagem de um erro ou permitir uma exceção não esperada para encerrar o programa de forma abrupta. Além disso, alterações para corrigir tratamento de erros precisam fluir através de múltiplas funções `callbacks` e blocos `catch` são tediosos de se criar.

Várias tentativas de solucionar esse problema têm sido feitas com abstrações extras. Uma das mais bem sucedidas é chamada de `promises`. `Promises` encapsulam uma ação assíncrona em um objeto, que pode ser passado e instruído a fazer algo quando a ação finalizar ou falhar. Essa interface está definida para ser parte da próxima versão da linguagem Javascript mas já pode ser usada em forma de biblioteca.

A interface de `promises` não pode ser chamada de intuitiva, mas ela é poderosa. Esse capítulo irá apenas brevemente o descrever. Você pode encontrar uma documentação mais minunciosa em www.promisejs.org.
...
Mais uma vez, segue nosso `wrapper` para requisições GET, dessa vez retornando uma `promise`. Nos simplesmente a chamaremos de `get` desta vez.
...
No entanto, essa é apenas outra forma de expressar a mesma coisa que já haviamos expressado. E apenas quando você precisa encadear ações juntas que as `promises` fazem uma diferenca significante.

A chamada ao método `then` produz uma nova `promise`, a qual o resultado (o valor passado para o manipulador em caso de sucesso) depende do valor de retorno da primeira função que passamos para o `then`. Essa função pode retornar outra `promise` para indicar que mais tarefas assincronas estão sendo executadas. Neste caso, a própria`promise` retornado pelo `then` IRA esperar pela `promise` retornada pela função manipuladora, obtendo sucesso ou falhando com o mesmo valor quando for resolvida. Quando a função manipuladora retornar um valor que não seja uma `promise`, a `promise` retornada pelo `then` imediatamente retorna com sucesso com tal valor como seu resultado.

Isso significa que você pode usar `then` para transformar o resultado de uma `promise`. Por exemplo, o codigo a seguir retorna uma `promise` a qual o resultado é o conteudo de uma dada URL, montado como JSON:
...
A última chamada ao `then` não especificou um manipulador de falhas. Isso é permitido. O erro sera passado para a `promise` retornada pelo `then`, a qual é exatamente o que nós queremos - `getJSON` não sabe o que fazer quando algo acontece errado, mas provavelmente seu chamador sabe.

Como um exemplo que demonstra o uso de `promises`, nós iremos construir um programa que carrega um numero de arquivos JSON do servidor e, enquanto isso é feito, mostra o carregamento de palavras. Os arquivos JSON contem informações sobre pessoas, com links para arquivos que representam outras pessoas em propriedades como pai, mãe ou esposa.

Nós queremos recuperar o nome da mãe da esposa de example/bert.json. E caso algo errado ocorra, nós queremos remover o texto de carregamento e mostrar uma mensagem de erro no lugar. Segue um exemplo de como isso é feito com `promises`:
...
O programa acima é relativamente compacto e legivel. O método `catch` é similar ao `then`, exceto que ele apenas espera um manipulador de falha e entao passa pelo resultado não modificado em caso de sucesso. Assim como a clausula `catch` para uma bloco `try`, o controle seguirá seu fluxo normal após a falha ser capturada. Desta forma, o derradeiro `then`, o qual remove a mensagem de carregamento, será sempre executado, mesmo que algum erro ocorra.

Você pode pensar na interface de `promise` como uma implementacao de sua própria linguagem para controle de fluxo assincrono. As chamadas extras de métodos e funções de expressão necessarias para atingir a tarefa fazem o codigo parecer meio esquisito mas não mais esquisito se fosse necessario tratar todas excecoes na forma tradicional.
...
Programação assíncrona é traiçoeira. `Promises` são interfaces que tornam a programação assíncrona mais fácil ajudando
...
Envie requisições para recuperar os tres formatos deste recurso. Use o método `setRequestHeader` de seu objeto `XMLHttpRequest` para definir o valor do cabeçalho chamado `Accept` para um dos tipos de mídia fornecidos anteriormente. Certifique-se de definir o valor do cabeçalho após chamar o método `open` mas antes de chamar o método `send`.
...
Para trabalhar com múltiplos promises utilizamos um método chamado `all` que, dado um array de `promises`, retorna uma `promise` que espera por todos os `promises` do array acabarem. Se obtiver sucesso, resultará em um array com os valores resultantes. Se qualquer das `promises` do array falhar, a `promise` retornada pelo `all` falhará também (com o valor de falha da `promise` que falhou).

Tente implementar algo assim por sua conta com uma função chamada `all`.

Observe que após um `promise` ser resolvida (tendo sucesso ou mesmo falhado), ela pode ter sucesso ou falhar de novo, e outras chamadas para as funções que resolvem isso são ignoradas. Isto pode simplificar a maneira que você manuseia falhas em sua `promise`.
...
A função passada ao construtor `Promise` terá que chamar `then` em cada dos `promises` do array dado. Quando um deles obtêm sucesso, duas coisas precisam acontecer: O valor resultante precisa ser armazenado na posição correta de um array resultante, e devemos checar se este foi o ultimo `promise` pendente e terminar nossa própria `promise` caso tenha sido.

O último pode ser resolvido com um contador, o qual é iniciado com a largura do array de entrada do qual nos subtraimos 1 toda vez que um `promise` obtem sucesso. Quando o contador atingir 0, encerramos. Garanta que o array de entrada esteja vazio (e portanto, nenhuma `promise` jamais irá resolver) em conta.

Manusear falhas requer alguns esforços mas se torna extremamente simples. Apenas passe a função de falha para o `promise` responsável por encapsular aos `promises` do array de forma que uma falha em um deles dispare a falha para o `promise` encapsulador.
